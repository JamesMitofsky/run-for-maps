...
{
  visible && (
    <View className="bg-primary uw-entering-bounce-in uw-exiting-bounce-out size-20 rounded-xl" />
  );
}
...
{
  visible && (
    <View className="bg-primary uw-entering-fade-in uw-entering-duration-1000 uw-exiting-fade-out size-20 rounded-xl" />
  );
}
```
