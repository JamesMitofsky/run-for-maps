...
// drinking_water=no means not human-potable (dogs only); an explicit dog=yes on
// an otherwise-potable source means it serves both.
export function audienceFromTags(tags: Record<string, string>): Audience {
  if (tags.drinking_water === "no") return "dogs";
